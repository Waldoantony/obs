...
function listenDraw() {
  db.ref("draw").on("value", snap => {
    ...
    const stateInfo = STATE_CONFIG.find(s => s.key === (data.state || "FL"));
    if (stateInfo) {
      document.getElementById("spotlight").style.borderColor = stateInfo.color;
      document.getElementById("spotlight").style.boxShadow =
        `0 0 40px ${stateInfo.color}55, inset 0 0 60px rgba(0,0,0,0.4)`;
    }
