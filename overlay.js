...
// ---------------- Color helper ----------------
// Converts "#9b5de5" -> "rgba(155,93,229,ALPHA)" so we can tint whole
// state cards using each state's accent color from data.js.
...
function buildCards() {
  ...
  STATE_CONFIG.forEach((state, i) => {
    ...
    card.style.background =
      `linear-gradient(160deg, ${hexToRgba(state.color, 0.55)}, rgba(6,10,20,.94))`;
    ...
    state.fields.forEach(f => {
      ...
      badge.textContent = fieldIcon(f.label);
...
function listenDraw() {
  db.ref("draw").on("value", snap => {
    ...
    // Swap the spotlight background photo per featured state (data.js bgImage).
    // Falls back to the plain dark panel background when none is set.
    const centerPanel = document.getElementById("centerPanel");
    if (stateInfo.bgImage) {
      centerPanel.style.backgroundImage =
        `linear-gradient(180deg, rgba(1,7,16,.55), rgba(1,7,16,.88)), url("${stateInfo.bgImage}")`;
...
function listenMeta() {
  db.ref("meta").on("value", snap => {
    ...
    clockBadge.style.display = data.timerOn === false ? "none" : "flex";
  });
}
